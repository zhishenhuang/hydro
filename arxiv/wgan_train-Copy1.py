...
def wgan_train(netG,netD,\
    ...
    for epoch in range(num_epochs):
        try:
                ...
                if (global_step%print_every==0) and (len(D_losses)>0) :
                    ...
                    if save_cp:
                        try:
                            ...
                            pass
                        torch.save({'model_state_dict': netD.state_dict()}, dir_checkpoint + f'netD_wg_{noise_mode}_epoch_{epoch}.pt')
                        torch.save({'model_state_dict': netG.state_dict()}, dir_checkpoint + f'netG_wg_{noise_mode}_epoch_{epoch}.pt')
            ...
            if save_cp:
                torch.save({'model_state_dict': netD.state_dict()}, dir_checkpoint + f'netD_wg_{noise_mode}_epoch_{epoch}_fileind_{fileind}.pt')
                torch.save({'model_state_dict': netG.state_dict()}, dir_checkpoint + f'netG_wg_{noise_mode}_epoch_{epoch}_fileind_{fileind}.pt')
                np.savez(dir_checkpoint +f'wgan_train_track_{noise_mode}.npz',\
                                     g_loss=G_losses,d_loss=D_losses,Massdiffs=Massdiffs,\
                                     nrmse_train=nrmse_train,l1_train=l1_train,\
                                     nrmse_val=nrmse_val,l1_val=l1_val,\
