...
def validate(testfiles,netD,netG,dep=8,batchsize=2,seed=0,img_size=320, \
             device="cpu",testfile_num=100,weight_fid=10,fidelity_loss=nn.MSELoss(),\
...
def wgan_train(netG,netD,lrd=1e-5,lrg=2e-5,beta1=0.5,\
        ...
        try:
            ...
            while fileind < traintotal:
                ...
                if G_update_ind%update_G_every == 0:
                    ...
                    mass_real = compute_mass(real_cpu)
                    g_loss = - d_loss.mean() + weight_fid * fidelity_loss(mass_fake,mass_real)
            ...
            ############################
                if global_step%validate_every==0 :
                    massdiff,nrmse,l1err = validate(testfiles,netD,netG,dep=dep,batchsize=2,\
                                                    ...
                                                    weight_fid=weight_fid,device=device,testfile_num=testtotal,\
